...
namespace UnityEditor
{
	...
	namespace ProjectWindowCallback
	{
		...
		public class ProjectWindowUtil
		{
			...
			internal static void EndNameEditAction(EndNameEditAction action, int instanceId, string pathName,
			...
			public static void CreateScriptAssetFromTemplateFile(string templatePath, string defaultNewFileName)
			{
				...
				StartNameEditingIfProjectWindowExists(0, ScriptableObject.CreateInstance<DoCreateScriptAsset>(), defaultNewFileName,
					icon, templatePath);
			}
			...
			public static void StartNameEditingIfProjectWindowExists(int instanceID, EndNameEditAction endAction,
				string pathName, Texture2D icon, string resourceFile)
			{
			...
			internal static void GetAncestors(int instanceID, HashSet<int> ancestors)
			{
				...
				int mainAssetInstanceID =
					AssetDatabase.GetMainAssetOrInProgressProxyInstanceID(AssetDatabase.GetAssetPath(instanceID));
			...
			internal static bool DeleteAssets(List<int> instanceIDs, bool askIfSure)
			{
				...
				{
					EditorUtility.DisplayDialog(L10n.Tr("Cannot Delete"), L10n.Tr("Deleting the 'Assets' folder is not allowed"),
						L10n.Tr("Ok"));
					return false;
				}
				...
				if (!success)
				{
					EditorUtility.DisplayDialog(L10n.Tr("Cannot Delete"),
						L10n.Tr(
